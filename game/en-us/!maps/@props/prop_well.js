...
export async function well_triggerProp() {
    let it = item;
    console.log(chalk.yellow.italic(`You have found a well; water is scarce, and a spectral glow—standing out against the surroundings—shimmers on the little that remains. You feel an irresistible urge to retrieve whatever lies at the bottom! The well features a mechanism holding a bucket attached to a rope; the rope looks fragile and worn by time, and it might snap before reaching the bottom. ROLL A D6 TO DETERMINE YOUR CHANCE OF SUCCESS!`));
    prompt();
